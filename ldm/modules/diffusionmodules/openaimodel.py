...
from ldm.modules.attention import SpatialTransformer, SpatialTransformerV2, SpatialTransformerIPAIDPlus
from ldm.modules.spade import SPADE
from ldm.modules.IDBalance import IDBalance
...
class TimestepBlockV3(nn.Module):
    """
    Any module where forward() takes timestep embeddings as a second argument.
    """

    @abstractmethod
    def forward(self, x, emb, cond, context):
        """
        Apply the module to `x` given `emb` timestep embeddings.
        """
...
class TimestepEmbedSequentialIPA(nn.Sequential, TimestepBlock):

    def forward(self, x, emb, context=None,struct_cond=None, ip_scale=0.75, seg_cond=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformerIPAIDPlus):
                x = layer(x, context, ip_scale)
            elif isinstance(layer, TimestepBlockDual):
                assert struct_cond is not None
                x = layer(x, emb, struct_cond)
            elif isinstance(layer, TimestepBlockV3):
                assert struct_cond is not None
                x = layer(x, emb, struct_cond, context)
            else:
                x = layer(x)
        return x
...
class ResBlockV3(TimestepBlockV3):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of timestep embedding channels.
    ...
    def __init__(
        ...
        self.IDBalance = IDBalance(self.out_channels,semb_channels)

    def forward(self, x, emb, s_cond, context):
        """
        Apply the block to a Tensor, conditioned on a timestep embedding.
        :param x: an [N x C x ...] Tensor of features.
        :param emb: an [N x emb_channels] Tensor of timestep embeddings.
        ...
        """
        return checkpoint(
            self._forward, (x, emb, s_cond, context), self.parameters(), self.use_checkpoint
        )


    def _forward(self, x, emb, s_cond, context):
        if self.updown:
            in_rest, in_conv = self.in_layers[:-1], self.in_layers[-1]
            h = in_rest(x)
            h = self.h_upd(h)
        ...
        h = h + self.IDBalance(h,s_cond,ref_cond)
        return h
...
class UNetModelRestorerID(nn.Module):
    """
    ...
    def __init__(
                    ...
                    if not exists(num_attention_blocks) or nr < num_attention_blocks[level]:
                        layers.append(
                            AttentionBlock(
                                ...
                                num_head_channels=dim_head,
                                use_new_attention_order=use_new_attention_order,
                            ) if not use_spatial_transformer else SpatialTransformerIPAIDPlus(
                                ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim
                            )
                        )
                self.input_blocks.append(TimestepEmbedSequentialIPA(*layers))
                self._feature_size += ch
                input_block_chans.append(ch)
            if level != len(channel_mult) - 1:
                out_ch = ch
        ...
        if legacy:
            #num_heads = 1
            dim_head = ch // num_heads if use_spatial_transformer else num_head_channels
        self.middle_block = TimestepEmbedSequentialIPA(
            ResBlockV3(
                ch,
                time_embed_dim,
                dropout,
            ...
            AttentionBlock(
                ...
                num_head_channels=dim_head,
                use_new_attention_order=use_new_attention_order,
            ) if not use_spatial_transformer else SpatialTransformerIPAIDPlus(  # always uses a self-attn
                            ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim
                        ),
            ResBlockV3(
                ch,
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(self.num_res_blocks[level] + 1):
                ...
                if level and i == self.num_res_blocks[level]:
                    ...
                    ds //= 2
                self.output_blocks.append(TimestepEmbedSequentialIPA(*layers))
                self._feature_size += ch
    ...
    def forward(self, x, timesteps=None, context=None, struct_cond=None, ip_scale = 0.75, y=None,**kwargs):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        h = x.type(self.dtype)
        for module in self.input_blocks:
            h = module(h, emb, context, struct_cond,ip_scale)
            hs.append(h)
        
        h = self.middle_block(h, emb, context, struct_cond,ip_scale)
...
class EncoderUNetModelWT(nn.Module):
    ...
    def init_from_ckpt(self,path):
        ...
        print(f"EncoderUNetModelWT Restored from {path} with {len(missing)} missing and {len(unexpected)} unexpected keys")
        if len(missing) > 0:
