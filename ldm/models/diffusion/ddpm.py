from ldm.modules.encoders.modules import IPAdapterFaceIDPlus
import torch
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t,ip_scale=0.75,  c_concat: list = None, c_crossattn: list = None, struct_cond=None, seg_cond=None):
        if self.conditioning_key is None:
            ...
            if struct_cond is None:
                out = self.diffusion_model(x,t, context=cc,ip_scale=ip_scale)
                return out
            else:
                if seg_cond is None:
                    out = self.diffusion_model(x,t, context=cc,struct_cond = struct_cond, ip_scale=ip_scale)
                else:
...
class LatentDiffusionFaceBIR(DDPM):
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        self.lq, self.gt = self.Degrad.feed_data(im_gt)
    ...
    def forward(self, x, c, gt, *args, **kwargs):
        ...
        if self.test_gt:
            ...
            struc_c = self.structcond_stage_model(x, t_ori)   #['64':B*256*64*64,'32':B*256*32*32,'16':B*256*16*16,'8':B*256*8*8]
...
class LatentDiffusionRestorerID(DDPM):
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
        ...
        self.use_usm = use_usm
        self.mix_ratio = mix_ratio
        self.IPA_dropout = IPA_dropout
        self.structcond_dropout = structcond_dropout
        assert self.num_timesteps_cond <= kwargs['timesteps']
        # for backwards compatibility after implementation of DiffusionWrapper
        if conditioning_key is None:
            conditioning_key = 'concat' if concat_mode else 'crossattn'
        ...
        self.Adapter = IPAdapterFaceIDPlus()
        self.FrozeAdapter()
        ...
        param_list = []
        print('>>>>>>>>>>>>>>>>IPAdapter>>>>>>>>>>>>>>>>>>>>')
        for name, params in self.Adapter.named_parameters():
            if params.requires_grad:
                param_list.append(name)
        print(param_list)
    ...
    def FrozeAdapter(self):
        self.Adapter = self.Adapter.eval()
        self.Adapter.train = disabled_train
        for param in self.Adapter.parameters():
            param.requires_grad = False

    
    def UnFrozeIDBalance(self):
        for name, param in self.model.named_parameters():
            if 'IDBalance' in name:
                param.requires_grad = True
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        faceid_embeds = batch['faceid_embeds']
        ...
        self.lq = torch.clamp(self.lq, -1.0, 1.0).to(self.device)
        ...
        encoder_posterior_lq = self.encode_first_stage(x)
        ...
        out = [z_gt, z_lq, img_ref, faceid_embeds]
    ...
    def shared_step(self, batch, **kwargs):
        z_gt, z_lq, img_ref, faceid_embeds  = self.get_input(batch, self.first_stage_key)
        loss = self(z_gt, z_lq, img_ref, faceid_embeds)
        return loss
    ...
    def forward(self, z_gt, z_lq, img_ref, faceid_embeds, *args, **kwargs):
        index = np.random.randint(0, self.num_timesteps, size=z_gt.size(0))
        t = torch.from_numpy(index)
        t = t.to(self.device).long()

        t_ori = torch.tensor([self.ori_timesteps[index_i] for index_i in index])
        t_ori = t_ori.long().to(z_gt.device)

        text_cond=['']*z_gt.size(0)
        ...
        if self.model.conditioning_key is not None:        
            ...
            if random.random()>self.IPA_dropout:                    #CFG training
                ...
                cond = torch.cat([semantic_c,uncon_ref_c],dim=1)
            if self.shorten_cond_schedule:  # TODO: drop this option  False
                tc = self.cond_ids[t].to(self.device)
                c = self.q_sample(x_start=c, t=tc, noise=torch.randn_like(c.float()))
        if self.test_gt:
            ...
            if random.random()>self.structcond_dropout:                #cfg training
                struc_c = self.structcond_stage_model(z_lq, t_ori)   #['64':B*256*64*64,'32':B*256*32*32,'16':B*256*16*16,'8':B*256*8*8]
            else:
                struc_c = self.structcond_stage_model(torch.zeros_like(z_lq), t_ori)

        loss,loss_dict = self.p_losses(z_gt, cond, struc_c, t, t_ori, *args, **kwargs)
    ...
    def apply_model(self, x_noisy, t, cond, struct_cond, ip_scale, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            cond['struct_cond'] = struct_cond
            x_recon = self.model(x_noisy, t,ip_scale, **cond)
    ...
    def p_losses(self, z_gt, cond, struc_cond,  t, t_ori, noise=None):
        noise = default(noise, lambda: torch.randn_like(z_gt))
        x_noisy = self.q_sample(x_start=z_gt, t=t, noise=noise)

        ip_scale = 0.75

        if self.mix_ratio > 0:
            if random.random() < self.mix_ratio:
                noise_new = default(noise, lambda: torch.randn_like(z_gt))
                noise = noise_new * 0.5 + noise * 0.5
                x_noisy = self.q_sample(x_start=z_gt, t=t, noise=noise)

        model_output = self.apply_model(x_noisy, t_ori, cond, struc_cond, ip_scale)
        ...
        if self.parameterization == "x0":
            ...
            target = self.get_v(z_gt, noise, t)
        else:
            raise NotImplementedError()
    ...
    def p_mean_variance(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        ip_scale = 0.75
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            model_out = self.apply_model(x, t_in, c, struct_cond=struct_cond,ip_scale = ip_scale, return_ids=return_codebook_ids)
        else:
            x_in = torch.cat([x] * 2)
            t_in_ = torch.cat([t_in] * 2)
            c_in = torch.cat([unconditional_conditioning, c])
            for key in struct_cond.keys():
                struct_cond[key] = torch.cat([struct_cond[key], struct_cond[key]], dim=0)
            e_t_uncond, e_t = self.apply_model(x_in, t_in_, c_in, struct_cond, ip_scale=ip_scale, return_ids=False).chunk(2)
            model_out = e_t_uncond + unconditional_guidance_scale * (e_t - e_t_uncond)
            return_codebook_ids=False
    ...
    def p_sample_loop(self, cond, struct_cond, shape, return_intermediates=False,
        ...
        for i in iterator:
            ...
            img = self.p_sample(img, cond, struct_cond=struct_cond_input,t=ts,
                                clip_denoised=self.clip_denoised,
                                quantize_denoised=quantize_denoised, t_replace=t_replace,
                                unconditional_conditioning=unconditional_conditioning,
                                unconditional_guidance_scale=unconditional_guidance_scale,
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        log = dict()
        z_gt, z_lq, ref_img, faceid_embeds = self.get_input(batch, self.first_stage_key,
                                           return_first_stage_outputs=False,
                                           force_c_encode=True,
                                           return_original_cond=False,
                                           bs=N, val=True)
        N = min(z_gt.shape[0], N)
        n_row = min(z_gt.shape[0], n_row)
        if self.test_gt:
            log["gt"] = z_gt
        else:
            log["inputs"] = z_lq
            log["reconstruction"] = self.decode_first_stage(z_gt)
            log["recon_lq"] = self.decode_first_stage(z_lq)

        positive_text_c = ["best quality, high quality"]*z_gt.shape[0]
        positive_semantic_c = self.cond_stage_model(positive_text_c)
        negative_text_c=["monochrome, lowres, bad anatomy, worst quality, low quality"]*z_gt.shape[0]
        negative_semantic_c = self.cond_stage_model(negative_text_c)

        ref_c,uncon_ref_c = self.Adapter(faceid_embeds, ref_img)
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                if self.time_replace is not None:
                    ...
                    cur_time_step = 1000
                samples, z_denoise_row = self.sample(cond=positive_cond,struct_cond=struct_cond, batch_size=N, 
								unconditional_conditioning=negative_cond, unconditional_guidance_scale=7.5,
								timesteps=cur_time_step, time_replace=self.time_replace, return_intermediates=True)
                # samples, z_denoise_row = self.sample(cond=c, struct_cond=struct_cond, ref_cond = ref_cond, batch_size=N, timesteps=cur_time_step, return_intermediates=True, time_replace=self.time_replace)
            x_samples = self.decode_first_stage(samples)
            log["samples"] = x_samples
            if plot_denoise_rows:
                denoise_grid = self._get_denoise_row_from_list(z_denoise_row)
            ...
            if inpaint:
                ...
                with self.ema_scope("Plotting Outpaint"):
                    samples, _ = self.sample_log(cond=c, batch_size=N, ddim=use_ddim,eta=ddim_eta,
                                                ddim_steps=ddim_steps, x0=z_gt[:N], mask=mask)
                x_samples = self.decode_first_stage(samples.to(self.device))
                log["samples_outpainting"] = x_samples
