...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("--name",type=str,const=True, default="RestorerID",nargs="?",help="postfix for logdir",)
    parser.add_argument("--resume",type=str,const=True,default="",nargs="?",help="resume from logdir or checkpoint in logdir",)
    parser.add_argument("--base",nargs="*",metavar="configs/v15/v15-RestorerID.yaml",help="paths to base configs. Loaded from left-to-right. "
             "Parameters can be overwritten or added with command-line options of the form `--key value`.",default=list(),)
    parser.add_argument("--train",type=str2bool,const=True,default=True,nargs="?",help="train",)
    ...
    parser.add_argument("--seed",type=int,default=53,help="seed for seed_everything",)
...
if __name__ == "__main__":
    ...
    checkpoint_callback = ModelCheckpoint(dirpath= ckptdir,                   
                                        filename= 'model_{epoch}_{step:06d}', 
                                        verbose= True,                      
                                        save_last= False,
